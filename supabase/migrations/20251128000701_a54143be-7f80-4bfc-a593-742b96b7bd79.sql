-- Fix the email notification trigger to use correct HTTP response field
DROP FUNCTION IF EXISTS public.send_personalized_email() CASCADE;
...
BEGIN
  ...
  LOOP
    ...
    -- Log if there's an error (using correct field name 'status')
    IF http_response.status NOT BETWEEN 200 AND 299 THEN
      RAISE WARNING 'Failed to send email to %: Status %, Response: %', recipient_email, http_response.status, http_response.content;
    END IF;
  END LOOP;
