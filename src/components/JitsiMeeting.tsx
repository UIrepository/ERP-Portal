...
export const JitsiMeeting = ({
  ...
  const openInNewTab = useCallback(() => {
    ...
    if (isHost) {
      toast.info('Please log in or start the meeting in the new tab.');
    } else {
  ...
  const initializeJitsi = useCallback(async () => {
    ...
    connectionTimeoutRef.current = setTimeout(() => {
      if (isLoadingRef.current) {
        setLoadingState(false); // Don't show hard error, just show fallback
        setConnectionError(true); 
        isInitializingRef.current = false;
      }
            ...
            {isHost ? (
               <div className="space-y-4">
                ...
                >
                  ...
                  Launch Class (New Tab)
                </Button>
                <p className="text-xs text-yellow-500/80 bg-yellow-500/10 p-3 rounded-lg text-left">
                  <strong>Teacher Note:</strong> Use this button to open the class in a new tab and authenticate. This window will connect automatically once you do.
                </p>
               </div>
            ) : (
              <Button 
            ...
            {!showFallbackPrompt ? (
              ...
              <>
                ...
                <p className="text-white text-lg font-semibold">
                    {isHost ? "Authentication Required" : "Waiting for Teacher..."}
                </p>
                <p className="text-gray-400 text-sm mt-2 mb-6">
                    {isHost 
                        ? "Please complete the setup in the new tab." 
