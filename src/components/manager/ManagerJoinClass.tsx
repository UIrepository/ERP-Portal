...
export const ManagerJoinClass = () => {
  ...
  const { data: manager, isLoading: isLoadingManager } = useQuery<Manager | null>({
    queryKey: ['managerAssignments', profile?.user_id],
    queryFn: async () => {
      if (!profile?.user_id) return null;
      const { data, error } = await supabase
        .from('managers')
        .select('id, assigned_batches')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', 
        ...
        () => {
          queryClient.invalidateQueries({ queryKey: ['classAttendanceManager', selectedClassForAttendance.id] });
        }
      )
      .subscribe();
  ...
  // Filter schedules for today based on manager's assigned batches
  ...
  const isLoading = isLoadingManager || isLoadingSchedules;
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Monitor Classes</h1>
        <p className="text-muted-foreground">Today's classes • {format(new Date(), 'EEEE, MMMM d, yyyy')}</p>
        <p className="text-sm text-muted-foreground mt-1">
          Managing batches: {manager.assigned_batches?.join(', ') || 'None'}
        </p>
      </div>
      ...
      {todaysClasses.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center mt-2">
              There are no scheduled classes for your batches today.
            </p>
          </CardContent>
        </Card>
      )}
