...
export const AuthPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <Card className="relative z-10 w-full max-w-sm bg-white rounded-xl shadow-lg overflow-hidden animate-fade-in-up">
        <CardHeader className="text-center pt-8 px-6 pb-4 bg-white text-gray-800 relative">
          <div className="mt-6 flex justify-center"> {/* Added mt-6 for top margin */}
            <img src="/logoofficial.png" alt="Unknown IITians Logo" className="h-16 w-16" /> {/* Adjusted size to 64x64px */}
          </div>
          <CardTitle className="text-xl font-bold text-center mb-2"> {/* Adjusted font size and weight */}
            Welcome to Unknown IITians
          </CardTitle>
          <CardDescription className="text-sm font-normal leading-relaxed px-4 text-center text-gray-700"> {/* Adjusted font size, weight, line height, padding, and color */}
        ...
        <CardContent className="p-6"> {/* Adjusted padding */}
          <div className="space-y-4">
            <Button 
              type="button" 
              className="w-full text-base font-semibold py-2.5 rounded-lg shadow-md mt-4 flex items-center justify-center gap-2" /* w-full, text-base, font-semibold, py-2.5, rounded-lg, shadow-md, mt-4, gap-2 */
              onClick={handleGoogleAuth}
              disabled={isGoogleLoading}
            >
              {isGoogleLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {/* Adjusted icon size */}
              <svg className="h-4 w-4" viewBox="0 0 24 24"> {/* Adjusted icon size */}
