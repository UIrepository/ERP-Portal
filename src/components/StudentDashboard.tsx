...
import { Skeleton } from '@/components/ui/skeleton'; // Corrected import: Added Skeleton
...
interface UIKiPadhaiContent {
  ...
  category?: string;
  link: string;
  is_active: boolean; // This column is now assumed to exist in dpp_content
  created_at: string;
  batch: string;
  subject: string;
}
...
export const StudentUIKiPadhai = () => {
  ...
  const { data: premiumContent, isLoading: isLoadingPremiumContent } = useQuery<UIKiPadhaiContent[]>({
    ...
    queryFn: async (): Promise<UIKiPadhaiContent[]> => {
        ...
        let query = supabase
            .from('dpp_content') // Assumed to hold UI Ki Padhai content, and now has 'is_active' column
            .select('*')
            .eq('is_active', true);
