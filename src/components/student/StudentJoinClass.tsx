...
import { format, isToday, parse, isBefore, isAfter, getDay } from 'date-fns';
...
export const StudentJoinClass = () => {
  ...
  // Use a state for current time to force re-renders every minute (keeps Live/Upcoming status accurate)
  ...
  useEffect(() => {
    // Update the clock every minute so "Live" status updates automatically
  ...
  const { data: enrollments, isLoading: isLoadingEnrollments } = useQuery<UserEnrollment[]>({
    ...
    queryFn: async () => {
      // Support checking both profile ID and auth User ID to be safe
  ...
  const todaysClasses = useMemo(() => {
    ...
    // Get clean values for "Today"
    const now = new Date();
    const todayDayOfWeek = getDay(now); // 0 = Sunday
    const todayDateString = format(now, 'yyyy-MM-dd'); // "2024-05-20"
    ...
    return schedules.filter(schedule => {
      // 1. Check if student is enrolled in this batch+subject
      const isEnrolled = enrolledCombinations.has(`${schedule.batch}|${schedule.subject}`);
      if (!isEnrolled) return false;
      
      // 2. Check if this schedule is for today
      if (schedule.date) {
        // FIXED: String comparison avoids timezone offset bugs causing classes to vanish
  ...
  }, [schedules, enrollments]); // We rely on the hook above to trigger re-renders on date change

  // Categorize classes
  const { liveClasses, upcomingClasses, completedClasses } = useMemo(() => {
    // Use the state 'currentTime' here to ensure this recalculates periodically
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         {cls.date && <Badge variant="outline" className="text-xs">Extra Class</Badge>}
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         {cls.date && <Badge variant="secondary" className="text-xs">Extra Class</Badge>}
                      </div>
                      <p className="text-sm mt-2 flex items-center gap-2">
                        <Clock className="h-4 w-4" />
                        {formatTime(cls.start_time)} - {formatTime(cls.end_time)}
